package com.shiping.mariobros.Sprites;
...
public class Mario extends Sprite {
    ...
    public enum State { FALLING, JUMPING, STANDING, RUNNING, GROWING, DEAD, FLYING };
    ...
    public Mario (PlayScreen screen) {
        ...
        frames.add(new TextureRegion(screen.getAtlas().findRegion("big_mario"), 240, 0, 16, 32)); // 240 because 15th frame * 16 pixels
        frames.add(new TextureRegion(screen.getAtlas().findRegion("big_mario"), 0, 0, 16, 32)); // 240 because 15th frame * 16 pixels
        frames.add(new TextureRegion(screen.getAtlas().findRegion("big_mario"), 240, 0, 16, 32)); // 240 because 15th frame * 16 pixels
        frames.add(new TextureRegion(screen.getAtlas().findRegion("big_mario"), 0, 0, 16, 32)); // 240 because 15th frame * 16 pixels
        ...
        // create texture region for mario life lost
    ...
    public State getState () {
        ...
        else if (b2body.getLinearVelocity().y > 0 || (b2body.getLinearVelocity().y < 0 && previousState == State.JUMPING)) return State.JUMPING;
    ...
    public void defineBigMario() {
        ...
        BodyDef bdef = new BodyDef();
        bdef.position.set(currentPosition.add(0, 10 / MarioBros.PPM)); // add because bigMario is taller and needs to be displaced accordingly
        bdef.type = BodyDef.BodyType.DynamicBody;
        b2body = world.createBody(bdef);
